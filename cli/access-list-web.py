...
class WebHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed.path == '/':
            if app_state is not None:
                self._html(pages_handlers._render_home(app_state))
            else:
    ...
    def _serve_static(self, path: str) -> bool:
        resource = path[len('/static/') :].lstrip('/')
        if not resource:
            return False
        if resource == 'fonts/local.css':
            app_state = getattr(self.server, 'app_state', None)
            css = getattr(app_state, 'font_css', None) or "/* no local fonts configured */\n"
            self._send_bytes(css.encode('utf-8'), 'text/css; charset=utf-8', 'max-age=60, must-revalidate')
            return True
        app_state = getattr(self.server, 'app_state', None)
        if app_state is not None:
            for font in getattr(app_state, 'font_files', []):
                if font.url_path == path:
                    try:
                        data = font.file_path.read_bytes()
                    except Exception:
                        break
                    content_type = mimetypes.guess_type(str(font.file_path))[0] or 'font/ttf'
                    self._send_bytes(data, content_type, 'public, max-age=604800')
                    return True
        payload = self._load_static_asset(resource)
        if payload is None:
            return False
        content_type = mimetypes.guess_type(resource)[0] or 'application/octet-stream'
        self._send_bytes(payload, content_type, 'max-age=300, public')
        return True

    def _load_static_asset(self, name: str) -> Optional[bytes]:
        try:
            return resources.read_binary('webui.static', name)
        except (FileNotFoundError, IsADirectoryError, ModuleNotFoundError):
            pass
        file_path = Path(__file__).resolve().parent / 'webui' / 'static' / name
