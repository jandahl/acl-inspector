...
"      for(const id of ['inspect','old','new','pkt_src','pkt_dst','findq']){const el=document.getElementById(id); if(el){el.addEventListener('input',fetchSuggest);}}\n"
            ...
            "    }\n"
            "    function loadState(){\n"
            "      try{const st=JSON.parse(storageGet('acl_state','{}')||'{}'); if(!st||typeof st!=='object') return; if(st.vendor){document.getElementById('vendor').value=st.vendor;} toggleVendor(); if(st.config&&document.getElementById('config')){document.getElementById('config').value=st.config;} if(st.config_ftg&&document.getElementById('config_ftg')){document.getElementById('config_ftg').value=st.config_ftg;} if(st.mode){document.getElementById('mode').value=st.mode;} toggleMode(); if(st.inspect&&document.getElementById('inspect')){document.getElementById('inspect').value=st.inspect;} if(st.old&&document.getElementById('old')){document.getElementById('old').value=st.old;} if(st.new&&document.getElementById('new')){document.getElementById('new').value=st.new;} if(st.findq&&document.getElementById('findq')){document.getElementById('findq').value=st.findq;} if(st.pkt_src&&document.getElementById('pkt_src')){document.getElementById('pkt_src').value=st.pkt_src;} if(st.pkt_dst&&document.getElementById('pkt_dst')){document.getElementById('pkt_dst').value=st.pkt_dst;} if(st.proto){document.querySelector(\"select[name='proto']\").value=st.proto;} if(typeof st.include_any==='boolean' && document.getElementById('include_any')){document.getElementById('include_any').checked=st.include_any;} if(typeof st.fuzzy==='boolean'){document.getElementById('fuzzy').checked=st.fuzzy;} if(st.dport){document.querySelector(\"input[name='dport']\").value=st.dport;}}catch(e){}\n"
            "    }\n"
            ...
            "    applyTheme(); loadState(); toggleVendor(); toggleMode(); attachTypeahead(); attachStateHandlers(); refreshMeta(); const hlOn=(storageGet(HL_KEY,'on')||'on')==='on'; highlightAll(hlOn); if(hlToggle){hlToggle.checked = hlOn;} renderHistory(); saveState();\n"
            "  </script>\n"
            "</body></html>\n"
        )
